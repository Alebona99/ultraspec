...
export function cmdReopen(env: UsEnv, args: string[]): string {
  const phase = args[0] ?? "";
  const rest = args.slice(1);
  let reason = "";
  for (let i = 0; i < rest.length; i++) {
    const a = rest[i];
    if (a === "--reason") { reason = rest[i + 1] ?? ""; i++; }
    else throw new Error(`opzione sconosciuta: ${a}`);
  }
  if (!phase) throw new Error('uso: /ultraspec:reopen <phase> --reason "..."');
  if (!reason) throw new Error('reopen richiede --reason "..." (viene registrato nella history)');
  ...
  if (pi === -1) throw new Error(`fase sconosciuta: ${phase}`);
  ...
  patchState(env, (s) => {
    ...
    for (const [key, value] of Object.entries(gates)) {
      ...
      if (gi === -1 || gi >= pi) gates[key] = { ...value, human_approved: false };
    }
    return {
      ...s,
      phase,
