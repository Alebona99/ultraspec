...
export function cmdApprove(env: UsEnv, args: string[]): string {
  const phase = args[0] ?? "";
  if (!phase) throw new Error("uso: /ultraspec:approve <phase>");
  if (!stateActive(env)) throw new Error(NO_ACTIVE);
  const { state } = readState(env);
  if (!state!.gates[phase]) throw new Error(`la fase '${phase}' non ha un gate da approvare`);
  patchState(env, (s) => ({
    ...s,
