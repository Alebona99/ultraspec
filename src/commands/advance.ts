...
function phaseAfter(env: UsEnv, phase: string): string | undefined {
  const idx = env.config.phase_order.indexOf(phase);
  if (idx === -1) return undefined;
  return env.config.phase_order[idx + 1];
}

export function cmdAdvance(env: UsEnv, args: string[]): string {
  if (!stateActive(env)) throw new Error(NO_ACTIVE);
  const { state } = readState(env);
  const s = state!;
  const cur = s.phase;
  const nxt = phaseAfter(env, cur);
  if (!nxt) throw new Error(`sei già alla fase finale ('${cur}')`);
  ...
  if (env.config.gates[cur]?.requires_approval === true) {
    if (s.gates[cur]?.human_approved !== true) {
      throw new Error(
        `avanzamento rifiutato: il gate '${cur}' richiede l'approvazione dell'utente (/ultraspec:approve ${cur}). /ultraspec:advance non approva.`
      );
    }
  }
