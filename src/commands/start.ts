...
import type { GateState, Track, UsEnv, UsState } from "../types.js";
import { cfg, nowIso, readState, writeState } from "../lib/state.js";

function gatesFromConfig(env: UsEnv): Record<string, GateState> {
...
export function cmdStart(env: UsEnv, args: string[]): string {
  const name = args[0] ?? "";
  ...
  for (let i = 0; i < rest.length; i++) {
    const a = rest[i];
    if (a === "--track") { track = rest[i + 1] ?? ""; i++; }
    else if (a === "--harness") { harness = rest[i + 1] ?? ""; i++; }
    else throw new Error(`opzione sconosciuta: ${a}`);
  }
  if (!name) throw new Error("manca il nome del workflow");
  ...
  if (valid && existing) {
    throw new Error(
      `esiste già un workflow attivo: ${existing.workflow} (fase ${existing.phase}). Chiudilo con /ultraspec:archive o usa un git worktree.`
    );
  }
