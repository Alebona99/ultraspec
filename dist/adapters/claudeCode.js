...
export function handleClaudeCodeHook(core, cwd, nativePayload) {
    ...
    const emitJson = (hookEventName, context) => JSON.stringify({ hookSpecificOutput: { hookEventName, additionalContext: context } });
    if (core === "gate") {
        if (d.decision === "deny")
            return { exitCode: 2, stdout: "", stderr: d.reason ?? "blocked by ultraspec" };
    ...
    if (core === "stop") {
        if (d.decision === "block" || d.decision === "nudge")
            return { exitCode: 2, stdout: "", stderr: d.reason ?? "ultraspec: fase non completata" };
        return { exitCode: 0, stdout: "", stderr: "" };
    }
    // nudge
