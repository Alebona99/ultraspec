function finish(harness, raw, partial) {
    return {
        event: partial.event ?? "ignore",
        harness,
...
function claudeCode(raw) {
    ...
    else if (hev === "SessionStart")
        partial = { event: "session_start" };
    else if (hev === "UserPromptSubmit")
        partial = { event: "user_prompt" };
    else if (hev === "Stop" || hev === "SubagentStop")
        partial = { event: "stop" };
    else if (hev === "PreCompact")
        partial = { event: "pre_compact" };
...
function openCode(raw) {
    ...
    else if (kind === "session.start" || kind === "session_start")
        partial = { event: "session_start" };
    else if (kind === "session.idle" || kind === "stop")
        partial = { event: "stop" };
    return finish("opencode", raw, partial);
}
function genericGit(raw) {
    return finish("generic-git", raw, { event: "pre_bash", command: raw?.command ?? "git commit" });
}
function passthrough(raw, harness) {
    return finish(harness, raw, {
        event: raw?.event ?? "ignore",
        target_path: raw?.target_path ?? null,
        command: raw?.command ?? null,
    });
}
export function normalizeEvent(harness, raw) {
