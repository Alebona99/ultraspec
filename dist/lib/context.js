...
import { cfg, patchState } from "./state.js";
import { emit, phaseAllowsCode } from "./decision.js";
export function artifactsDirAbs(env, state) {
...
export function missingArtifacts(env, state, phase) {
...
export function gateApproved(env, state, phase) {
    const gateCfg = env.config.gates[phase];
    if (!gateCfg?.requires_approval)
        return true;
    return state.gates[phase]?.human_approved === true;
}
export function phaseBannerShort(env, state) {
    ...
    const lines = [`[ultraspec] workflow=${workflow}  track=${track}  phase=${phase}`];
    if (phaseAllowsCode(env, state))
        lines.push("Modifiche a codice: CONSENTITE in questa fase.");
    else
        lines.push(`BLOCCATO in questa fase: modifiche a codice di prodotto (fino alla fase "${from}") e git commit/push (consentiti in: ${allowed}).`);
    if (nextcmd) {
...
export function sessionLogSummary(state) {
    if (!state.session_log.length)
        return "";
...
export function latestHandoff(env, state) {
...
export function recentHandoffBlob(env, state) {
...
export function memoryBlob(env) {
    if (!env.config.memory?.enabled)
        return "";
