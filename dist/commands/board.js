    ...
    if (stateActive(env)) {
        ...
        if (fs.existsSync(adir)) {
            ...
            for (const req of Array.from(required).sort()) {
                if (!fs.existsSync(path.join(adir, req)))
                    lines.push(`  [ ] ${req}  (richiesto da un gate)`);
            }
        }
        else {
...
function allWorkflowsSection(env) {
...
function testsSection(env) {
    ...
    for (const [file, cmd] of checks) {
        if (fs.existsSync(path.join(env.root, file)))
            lines.push(`  progetto:     ${cmd}`);
    }
    return lines;
}
function handoffSection(env) {
    const lines = ["", "--- Handoff ---"];
...
export function cmdBoard(env, args) {
